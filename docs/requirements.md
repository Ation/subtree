...
- **R030019** — After a successful `create`, Subtree opens the new sublime-project file in a new window and leaves the originating window open.
...
- **R030022** — Warnings produced by [R030021](#3-create-operation) and [R030023](#3-create-operation) do not roll back [R030014](#3-create-operation)–[R030018](#3-create-operation). The new worktree, sublime-project file, and config entry remain. Any warnings are surfaced to the user in a single end-of-operation message before [R030019](#3-create-operation) opens the new project.
- **R030023** — After [R030021](#3-create-operation) finishes, `create` performs a [path rewrite](glossary.md#path-rewrite) over the new worktree: every occurrence of the source worktree directory's absolute path is replaced by the new worktree directory's absolute path. The pass runs once, after all copying, and applies to:
  1. **Candidates.** Every file tracked in the new worktree's git index (`git -C <new-worktree-dir> ls-files -z`), plus every file beneath each directory that [R030021](#3-create-operation) copied successfully. Entries that R030021 skipped are not candidates. A file reachable both ways is processed once.
  2. **Eligibility.** A candidate is a [rewritable file](glossary.md#rewritable-file) only if it is a regular, non-symlink file whose name ends in one of the extensions listed in `REWRITE_FILE_EXTENSIONS` in `subtree.py` (initially `.md`, `.txt`, `.sh`, `.bash`, `.py`). Matching is case-sensitive, so `NOTES.MD` is not rewritten, and a name without an extension (e.g. `.venv/bin/activate`) never matches. The list is a source-code constant, not a `subtree_config.json` setting.
  3. **Substitution.** The file's bytes are searched for the UTF-8 encoding of the source worktree path. An occurrence is replaced only when the byte that follows it cannot continue a path component — that is, when it is not an ASCII letter, digit, `_`, `-`, or `.`; end of file counts as a boundary. This keeps `<root>/worktrees/master2`, `master-old`, and `master.bak` intact when the source worktree is `master`. No other byte is altered, so line endings, encodings, and binary content survive unchanged. The file is written back only if at least one occurrence was replaced, and its mode is preserved.
  4. **Scope.** Only the source worktree's own path is replaced. Absolute paths of other worktrees, including the [main worktree](glossary.md#main-worktree) when it is not the source, are left as they are. This is deliberately narrower than a rewrite of any `<root>/worktrees/<name>` prefix.
  5. **Failures.** If `git ls-files` fails, the tracked-file half of the pass is skipped with a single warning and the copied-directory half still runs. An `OSError` while reading or writing one file yields a warning naming that file and the pass continues. Warnings join those of [R030021](#3-create-operation) per [R030022](#3-create-operation).
  6. **Reporting.** When at least one file was rewritten, `create` posts the count to the status bar. When the count is zero it says nothing. Rewritten tracked files appear as modified in `git status` in the new worktree; this is expected and is not reverted.
...
- **R040019** — After a successful `open`, Subtree opens the new sublime-project file in a new window and leaves the originating window open (mirroring [R030019](#3-create-operation)).
...
- **R040022** — Mirrors [R030022](#3-create-operation): warnings produced by [R040021](#4-open-operation) and [R040023](#4-open-operation) do not roll back the operation; they are surfaced in a single end-of-operation message before [R040019](#4-open-operation).
- **R040023** — Mirrors [R030023](#3-create-operation): after [R040021](#4-open-operation) finishes, `open` performs the same [path rewrite](glossary.md#path-rewrite) over the new worktree, with the same candidate set, eligibility rule, boundary-aware substitution, failure handling, and status reporting. The path being replaced is that of the **template-source worktree** picked in [R040013](#4-open-operation) — the same worktree that supplied the project template ([R040017](#4-open-operation)) and the copied directories ([R040021](#4-open-operation)) — not the main worktree that served as git's working directory in [R040015](#4-open-operation) / [R040016](#4-open-operation).
...
- **R060017** — Cleanup: `remove` recursively walks the worktree directory; for every directory that contains a Docker Compose file (any of `docker-compose.yml`, `docker-compose.yaml`, `compose.yml`, `compose.yaml`), it runs `docker compose down -v` in that directory to tear down the stack. `down` is run **with** `-v`, so named volumes and the data they hold are removed along with the stack — this is destructive and not reversible. Non-zero exit from `docker compose down` is ignored (taken as "no running stack to remove"). If the `docker` executable is not on `PATH`, this cleanup is skipped with a status message; the remainder of `remove` proceeds. This cleanup is independent of the `pipenv` cleanup ([R060012](#6-remove-operation)) — a missing `pipenv` does not skip the compose teardown, and a missing `docker` does not skip the pipenv teardown. Both run before the git worktree is removed ([R060013](#6-remove-operation)).
...
`prune` reconciles [`subtree_config.json`](schemas/subtree_config.md) **and** the [`sublime_projects/`](structure.md) directory with the filesystem. It does two things: (1) it removes managed worktree entries whose [worktree directory](glossary.md#worktree-directory) has disappeared (e.g. deleted outside Subtree, or via `git worktree remove` run by hand) and cleans up their leftover project and workspace files; and (2) it deletes any [orphaned project/workspace file](glossary.md#orphaned-projectworkspace-file) left in `sublime_projects/` that no surviving `worktrees[]` entry references. Unlike [`remove`](#6-remove-operation), `prune` never touches a live worktree or runs any git command — every entry it acts on already has no directory on disk, so there is no working-tree state to lose and no pre-check to run.

- **R080001** — `prune` may only run from a Sublime window that has a folder open; it operates on `window.folders()[0]`. It errors if no folder is open.
- **R080002** — `prune` locates the [root directory](glossary.md#root-directory) by walking upward from the operating folder until it finds a directory containing [`subtree_config.json`](schemas/subtree_config.md) (same algorithm as [R030002](#3-create-operation)).
- **R080003** — `prune` aborts without modification if no `subtree_config.json` is found.
- **R080004** — `prune` aborts without modification if `subtree_config.json` cannot be parsed as JSON matching the [Subtree config schema](schemas/subtree_config.md).
- **R080005** — `prune` identifies the **stale entries**: every entry in `worktrees[]` whose directory `<root>/worktrees/<name>/` does not exist on disk, **except** `meta_information.main_worktree`, which is never pruned ([R010001](#1-restrictions-and-general-requirements) forbids deleting the main worktree or its project file, even if its directory is somehow missing).
- **R080006** — If there are neither stale entries ([R080005](#8-prune-operation)) nor orphaned files ([R080011](#8-prune-operation)), `prune` makes no modification and reports that there is nothing to prune.
- **R080007** — For each stale entry, `prune` deletes the entry's [sublime-project file](schemas/sublime_project.md) at `<root>/sublime_projects/<project_file>` if it exists, and the matching workspace file — the same basename with `.sublime-project` replaced by `.sublime-workspace` — if it exists (mirroring the workspace cleanup in [R060014](#6-remove-operation)).
- **R080008** — `prune` removes every stale entry from `worktrees[]` in [`subtree_config.json`](schemas/subtree_config.md) and rewrites the file with the same 4-space indent + trailing newline format used elsewhere. All other config content (`meta_information`, `settings`, unknown keys) is preserved. The config entry is dropped regardless of whether its project/workspace files could be deleted — the worktree directory is already gone ([R080005](#8-prune-operation)), so the entry is stale by definition.
- **R080009** — `prune` reports its work to the Sublime Text console log (`View → Show Console`): one line per pruned worktree naming it, one line per file deleted on its behalf, and one line per orphaned file deleted ([R080012](#8-prune-operation)). A single end-of-operation status message (or, when warnings occurred, a dialog) summarises the pruned entry names and the count of orphaned files deleted.
- **R080010** — A failure to delete a project or workspace file ([R080007](#8-prune-operation)) is reported as a warning and does not abort `prune`; the corresponding config entry is still removed ([R080008](#8-prune-operation)). A failure to rewrite `subtree_config.json` is reported with the root path so the user can inspect the partial state manually; `prune` does not attempt automatic rollback.
- **R080011** — `prune` identifies the **orphaned files**: every regular file directly under `<root>/sublime_projects/` whose name ends in `.sublime-project` or `.sublime-workspace` and which no entry of `worktrees[]` in [`subtree_config.json`](schemas/subtree_config.md) references. A `.sublime-project` file is referenced iff its basename equals some `worktrees[].project_file`; a `.sublime-workspace` file is referenced iff the basename obtained by replacing its trailing `.sublime-workspace` with `.sublime-project` equals some `worktrees[].project_file`. Orphaned files are identified against the config as it exists before the operation, so files belonging to a stale entry ([R080005](#8-prune-operation)) are **not** orphaned — they are handled by [R080007](#8-prune-operation). The [main worktree](glossary.md#main-worktree)'s project and workspace files are always referenced ([R010001](#1-restrictions-and-general-requirements)) and are never orphaned. Nested subdirectories under `sublime_projects/` are not scanned ([`sublime_projects/` is flat](structure.md)).
- **R080012** — `prune` deletes every orphaned file ([R080011](#8-prune-operation)). This touches only `sublime_projects/`; `subtree_config.json` is not affected (orphaned files have no config entry to remove). A failure to delete an orphaned file is reported as a warning and does not abort `prune` (as in [R080010](#8-prune-operation)).

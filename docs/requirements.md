...
`prune` reconciles [`subtree_config.json`](schemas/subtree_config.md) with the filesystem: it removes managed worktree entries whose [worktree directory](glossary.md#worktree-directory) has disappeared (e.g. deleted outside Subtree, or via `git worktree remove` run by hand) and cleans up their leftover project and workspace files. Unlike [`remove`](#6-remove-operation), `prune` never touches a live worktree or runs any git command — every entry it acts on already has no directory on disk, so there is no working-tree state to lose and no pre-check to run.
...
- **R080006** — If there are no stale entries, `prune` makes no modification and reports that there is nothing to prune.
...
- **R080009** — `prune` reports every pruned entry to the Sublime Text console log (`View → Show Console`): one line per pruned worktree naming it, and one line per file deleted on its behalf. A single end-of-operation status message (or, when warnings occurred, a dialog) summarises the pruned entry names.

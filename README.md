...
- **Subtree: Remove** — remove a worktree from a picker (the main worktree is excluded). Pre-checks for uncommitted changes, no-upstream branches with commits at risk, and ahead-of-upstream commits all gate the operation; pipenv environments are cleaned up if `pipenv` is on PATH, and Docker Compose stacks (`docker compose down -v`, which also removes named volumes and their data) are torn down for every compose file under the worktree if `docker` is on PATH. Removal calls `git worktree remove`, deletes the corresponding sublime-project file, and updates `subtree_config.json`.

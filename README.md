...
- **Subtree: Prune** — reconcile `subtree_config.json` with the filesystem: drop any worktree entry whose directory no longer exists on disk (the main worktree is never pruned) and delete its leftover `.sublime-project` / `.sublime-workspace` files. Runs no git commands and touches no live worktree; pruned entries and deleted files are reported to the Sublime Text console (`View → Show Console`).
